...
in the Synthesis and HyperSynthesisTransform helps getting faster decoding speed 
and better rate-distortion performance
...
class AnalysisTransform(tf.keras.layers.Layer):
  ...
  def build(self, input_shape):
    self._layers = [
        tfc.SignalConv2D(
            48, (9, 9), name="analysis/layer0", corr=True, strides_down=4,
  ...
  def build(self, input_shape):
    self._layers = [
        tf.keras.layers.Conv2DTranspose(
            32, (5, 5), strides=4, padding="same", use_bias=True,
            activation=tfc.GDN(inverse=True, name='igdn0'), name="synthesis/layer0"),
        tf.keras.layers.Conv2DTranspose(
            3, (5, 5), strides=4, padding="same", use_bias=True,
...
class HyperAnalysisTransform(tf.keras.layers.Layer):
  ...
  def build(self, input_shape):
    self._layers = [
       tfc.SignalConv2D(
            192, (7, 1), name="hyperanalysis/layer0", corr=True, strides_down=(4, 1),
            padding="same_zeros", use_bias=True, activation=tf.nn.relu),
       tfc.SignalConv2D(
            192*2, (1, 7), name="hyperanalysis/layer1", corr=True, strides_down=(1, 4),
            padding="same_zeros", use_bias=True, activation=tf.nn.relu),
       tfc.SignalConv2D(
            192, (2, 2), name="hyperanalysis/layer2", corr=True, strides_down=(1, 1),
            padding="same_zeros", use_bias=True,
            activation=None),
    ]
  ...
  def build(self, input_shape):
    self._layers = [
       tf.keras.layers.Conv2DTranspose(
            ...
            activation=tf.nn.relu, name="hypersynthesis/layer0"),
       tf.keras.layers.Conv2DTranspose(
            192, (1, 7), strides=(1,4),
            padding="same", use_bias=True,
            activation=tf.nn.relu, name="hypersynthesis/layer1"),
       tf.keras.layers.Conv2DTranspose(
            192, (1, 1), strides=(1,1),
            padding="same", use_bias=True,
            activation=None, name="hypersynthesis/clayer2"),
    ]
...
def compress_image(input_dir, output_dir, checkpoint_dir):
    ...
    hyper_synthesis_transform_1 = HyperSynthesisTransform()
    hyper_synthesis_transform_2 = HyperSynthesisTransform()
    ...
    sigma = hyper_synthesis_transform_1(z_hat)
    mean = hyper_synthesis_transform_2(z_hat)
    sigma = sigma[:, :y_shape[1], :y_shape[2], :]
    mean = mean[:, :y_shape[1], :y_shape[2], :]
    scale_table = np.exp(np.linspace(
        np.log(SCALES_MIN), np.log(SCALES_MAX), SCALES_LEVELS))
    conditional_bottleneck = tfc.LogisticConditional(sigma, scale_table, mean=mean)
...
def decompress_image(input_dir, output_dir, checkpoint_dir):
    ...
    revised_z_shape = tf.concat([z_shape, [192]], axis=0)
    z_hat = entropy_bottleneck.decompress(side_string, revised_z_shape, channels=192)
    sigma = hyper_synthesis_transform_1(z_hat)
    mean = hyper_synthesis_transform_2(z_hat)
    ...
    scale_table = np.exp(np.linspace(
      np.log(SCALES_MIN), np.log(SCALES_MAX), SCALES_LEVELS))
    conditional_bottleneck = tfc.LogisticConditional(
...
def parse_args():
  ...
  parser.add_argument(
      "--lambda", type=float, default=500, dest="lmbda",
...
def train2(args, train_dir):
  ...
  sigma = hyper_synthesis_transform_1(z_tilde)
  mean=hyper_synthesis_transform_2(z_tilde)
  scale_table = np.exp(np.linspace(
      np.log(SCALES_MIN), np.log(SCALES_MAX), SCALES_LEVELS))
  conditional_bottleneck = tfc.LogisticConditional(sigma, scale_table, mean=mean)
  ...
  with tf.train.MonitoredTrainingSession(
          ...
          save_checkpoint_secs=120, save_summaries_secs=60) as sess:
      ...
      while not sess.should_stop():
          ...
          print("step-{}: loss-{}, bpp-{},mse-{}, psnr-{}, msssim-{} using time {}".format
